...
def go_spider():
    ...
    origin_url = 'https://www.amac.org.cn/researchstatistics/report/zghybg/'
    url = 'https://www.amac.org.cn/researchstatistics/report/zghybg/'

    # 请求网址

    logging.info("正在抓取：NEW-M4-金融-资产管理-基金业：私募资管-证券期货经营机构私募资管产品")
    res = requests.request(method='get', url=url, headers=header, timeout=10)
    search_article_items = etree.HTML(res.content.decode()).xpath("//li[contains(string(),'证券期货经营机构私募资管产品备案月报')]")
    for items in search_article_items:
        ...
        if res_text.__contains__('证券期货经营机构私募资管产品备案月报'):
            res_data = {
                "title": res_pub_date + res_text,
                "data": f'''
                    检测到数据更新——
                    模板明细：NEW-M4-金融-资产管理-基金业：私募资管-证券期货经营机构私募资管产品
                    指标代码：1530097260
                    更新内容：{res_text}
                    更新时间：{res_pub_date}
...
def run():
    ...
    # 读取现有数据
    file_path = os.path.dirname(os.path.abspath(__file__)) + '\\NEW-M4-金融-资产管理-基金业：私募资管-证券期货经营机构私募资管产品.json'
